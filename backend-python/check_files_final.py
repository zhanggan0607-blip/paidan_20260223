...
cur.execute("SELECT id, file_path, file_size, original_filename FROM uploaded_file WHERE upload_date = '20260402' ORDER BY id DESC LIMIT 10")
rows = cur.fetchall()
print(f"Found {len(rows)} files")
for row in rows:
    print(f"  ID={row[0]}, path={row[1]}, size={row[2]}, filename={row[3]}")
