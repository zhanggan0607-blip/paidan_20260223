...
def generate_log_id(project_id: str | None, log_type: str, db: Session) -> str:
    """
    生成日志编号
    格式: 前缀-项目编号-年月日-序号
    ...
    project_part = project_id if project_id else "NO"
    base_id = f"{prefix}-{project_part}-{today}"
...
def get_today_maintenance_log(
    ...
    log = db.query(MaintenanceLog).filter(
        ...
        MaintenanceLog.created_by == user_info.name,
        MaintenanceLog.log_date == today
    ).first()
...
def create_maintenance_log(
    ...
    """
    today = datetime.now().strftime("%Y-%m-%d")
    if dto.log_date == today:
        existing_log = db.query(MaintenanceLog).filter(
            MaintenanceLog.is_deleted == False,
            MaintenanceLog.created_by == user_info.name,
            MaintenanceLog.log_date == today
        ).first()
...
def update_maintenance_log(
    ...
    today = datetime.now().strftime("%Y-%m-%d")
    log_date_str = log.log_date.strftime("%Y-%m-%d") if log.log_date else None
    
    if not user_info.is_manager and log_date_str != today:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="只能修改当天的维保日志"
        )
