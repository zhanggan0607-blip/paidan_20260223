...
def get_spare_parts_usage(
    ...
    for item in items:
        result_items.append({
            ...
            'issue_time': item.issue_time.strftime('%Y-%m-%d %H:%M:%S') if item.issue_time else None,
            'return_time': item.return_time.strftime('%Y-%m-%d %H:%M:%S') if item.return_time else None,
            'unit': item.unit,
            'status': item.status,
