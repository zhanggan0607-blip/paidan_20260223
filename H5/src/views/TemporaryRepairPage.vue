...
const fetchWorkList = async () => {
  ...
  try {
    const response = await api.get<unknown, ApiResponse<any[]>>('/work-plan/temporary-repair', { 
          ...
          <van-list :loading="loading" :finished="true">
            <van-cell-group inset>
              <van-cell 
                ...
                :label="`${formatDate(item.planDate)} | ${item.faultDescription}`"
                is-link
                @click="handleItemClick(item)"
              >
                <template #value>
                  <van-tag :type="item.status === WORK_STATUS.COMPLETED ? 'success' : 'warning'">
                    {{ item.status }}
                  </van-tag>
                </template>
              </van-cell>
