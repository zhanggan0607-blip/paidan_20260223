 ...
 * 安全存储工具
 * 提供加密的本地存储功能
 * 注意：此模块需要 crypto-js 依赖
 */

import CryptoJS from 'crypto-js'

const DEFAULT_SECRET_KEY = 'sstcp-maintenance-system-2026'

/**
 * 获取加密密钥
 */
const getSecretKey = (): string => {
  if (typeof import.meta !== 'undefined' && (import.meta as any).env?.VITE_STORAGE_SECRET_KEY) {
    return (import.meta as any).env.VITE_STORAGE_SECRET_KEY
  }
...
class SecureStorage {
  ...
  constructor(secretKey?: string) {
    this.key = secretKey || getSecretKey()
  }
